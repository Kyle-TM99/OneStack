...
values
    (1, 1, '대박이다.. 말이 돼?', '여기서 저렇게 했더니 요래 됐더라', sysdate()),
    (2, 2, '말이 돼?', '여기서 저렇게 했더니 요래 됐더라', sysdate()),
    (3, 3, '혹시 이거 아세요?', '이걸 하면 어떻게 된다고 했는데..', sysdate()),
    (4, 4, '헐, 그거 정말이야?', '진짜로 이렇게 되네.. 대박', sysdate()),
    (5, 5, '이거 진짜 신기하네', '이렇게 해서 성공한 사람들 많다고 하던데', sysdate()),
    (6, 6, '이건 좀 다르지 않나?', '생각보다 쉽고 재밌었어', sysdate()),
    (7, 7, '완전 쩐다!', '이렇게 하면 끝이야', sysdate()),
    (8, 8, '어떻게 이렇게 되는 거지?', '말도 안 되게 성공한 방법', sysdate()),
    (9, 9, '진짜 실화냐?', '이걸로 이렇게 바뀌었다는 거임?', sysdate()),
    (10, 10, '이거 대체 무슨 일이야?', '이 방법을 써봤는데 진짜로 효과 있더라', sysdate()),
    (11, 11, '그렇게 하면 될 줄 몰랐다', '이런 방법이 있었네..', sysdate()),
    (12, 12, '진짜 놀랐다', '이거 진짜로 효과 있었어', sysdate()),
    (13, 13, '이거 엄청 잘 됐어', '해보니까 진짜 괜찮았음', sysdate()),
    (14, 14, '이게 왜 이렇게 된 거지?', '믿을 수 없을 정도로 변했어', sysdate()),
    (15, 15, '이건 뭐지?', '한 번 해봤는데 너무 좋아서 깜짝 놀랐다', sysdate()),
    (16, 16, '이거 뭐야?', '이 방법대로 했더니 정말 효과 있네', sysdate()),
    (17, 17, '이게 바로 그거구나', '이렇게 된 이유가 있었네', sysdate()),
    (18, 18, '와 진짜 대단하다', '이걸로 뭔가 달라졌어', sysdate()),
    (19, 19, '이건 그냥 대박', '끝까지 해보니까 진짜 좋았음', sysdate()),
    (20, 20, '이건 다른 차원', '완전 다르게 변했어', sysdate()),
    (21, 21, '완전 신세계', '진짜 내가 해도 되나 싶었음', sysdate()),
    (22, 22, '이렇게 되다니..', '결과가 너무 좋았음', sysdate()),
    (23, 23, '이거 처음 해보는데', '와 진짜 해봤다니 믿을 수가 없네', sysdate()),
    (24, 24, '이걸로 됐다니', '끝까지 했더니 좋았어', sysdate()),
    (25, 25, '이거 정말 대박', '이렇게 해서 결과가 좋았음', sysdate()),
    (26, 26, '완전 달라졌다', '뭐가 달라진 건지 모르겠지만 대박', sysdate()),
    (27, 27, '이건 너무 신기한 방법', '한 번 해봤더니 너무 좋았어', sysdate()),
    (28, 28, '이렇게 될 줄 알았다', '사실 예상한 대로 됐다', sysdate()),
    (29, 29, '다시 시도해봤다', '다시 해보니까 훨씬 잘 됐어', sysdate()),
    (30, 30, '이건 진짜 꿈같다', '완전 잘 됐어', sysdate());
