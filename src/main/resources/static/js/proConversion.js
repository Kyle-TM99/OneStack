document.addEventListener('DOMContentLoaded', function () {
	...
	document.addEventListener('click', function (event) {
	    if (event.target && event.target.id === 'submitPortfolioBtn') {
	        ...
	        const portfolioFileInputs = Array.from(document.querySelectorAll('#portfolioFiles input[type="file"]'));
	        ...
	        portfolioFileInputs
	            ...
	            .forEach((fileInput, index) => {
	                formData.append(`portfolioFile${index + 1}`, fileInput.files[0]);
	            });
	    ...
	    window.addEventListener('message', function (event) {
	        ...
	        const { portfolioTitle, portfolioContent, surveyAnswers, thumbnailImagePath, portfolioFiles } = event.data;
	        ...
	        if (thumbnailImageDisplay) {
	            thumbnailImageDisplay.innerHTML = thumbnailImagePath
	                ? `<li><a href="${thumbnailImagePath}" target="_blank">${thumbnailImagePath}</a></li>`
	                : '<p>썸네일 이미지 없음</p>';
	        }
	        ...
	        if (portfolioFilesDisplay) {
	            portfolioFilesDisplay.innerHTML = portfolioFiles?.length
	                ? portfolioFiles.map(file => `<li><a href="${file}" target="_blank">${file}</a></li>`).join('')
	                : '<p>포트폴리오 파일이 없습니다.</p>';
	        }
